...
const APICodeDialog = ({ show, dialogProps, onCancel }) => {
    ...
    const codes = ['جاسازی', 'پایتون', 'جاوااسکریپت', 'cURL', 'اشتراک‌گذاری ربات گفت‌وگو']
...
output = query({
    "question": "سلام، چطور هستید؟",
})
`
        } else if (codeLang === 'جاوااسکریپت') {
...
query({"question": "سلام، چطور هستید؟"}).then((response) => {
...
`
    ...
    const getSVG = (codeLang) => {
        ...
        } else if (codeLang === 'جاسازی') {
            return EmbedSVG
        } else if (codeLang === 'cURL') {
            return cURLSVG
        } else if (codeLang === 'اشتراک‌گذاری ربات گفت‌وگو') {
...
# استفاده از فرم داده برای بارگذاری فایل‌ها
form_data = {
...
`
            ...
            return `// استفاده از FormData برای بارگذاری فایل‌ها
...
`
            ...
            return `curl ${baseURL}/api/v1/prediction/${dialogProps.chatflowid} \\
     -X POST \\
     -d '{"question": "سلام، چطور هستید؟", "overrideConfig": {${getConfigExamplesForCurl(configData, 'json')}}' \\
     -H "Content-Type: application/json"`
        }
        return ''
    }
...
`
    ...
    const getMultiConfigCodeWithFormData = (codeLang) => {
        if (dialogProps.isAgentflowV2) {
            if (codeLang === 'پایتون') {
                return `# تعیین چندین مقدار برای یک پارامتر پیکربندی با مشخص کردن شناسه گره
body_data = {
    "agentModelConfig": {
        "agentAgentflow_0": {
        ...
        >
            ...
            <DialogContent>
                <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                    <div style={{ flex: 80 }}>
                        <Tabs value={value} onChange={handleChange} aria-label='تب‌ها'>
                            {codes.map((codeLang, index) => (
                                <Tab
                                    icon={<img style={{ objectFit: 'cover', height: 15, width: 'auto' }} src={getSVG(codeLang)} alt='کد' />}
                                    iconPosition='start'
                                    key={index}
                                    label={codeLang}
                                    {...a11yProps(index)}
                    ...
                    <div style={{ flex: 20 }}>
                        <Available permission={'chatflows:update,agentflows:update'}>
                            <Dropdown
                                ...
                                value={dialogProps.chatflowApiKeyId ?? chatflowApiKeyId ?? 'انتخاب یک کلید API'}
                            />
                        </Available>
                    </div>
                </div>
                ...
                {codes.map((codeLang, index) => (
                    <TabPanel key={index} value={value} index={index}>
                        {(codeLang === 'جاسازی' || codeLang === 'اشتراک‌گذاری ربات گفت‌وگو') && chatflowApiKeyId && (
                            <>
                                <p>نمی‌توانید هنگام جاسازی یا اشتراک‌گذاری ربات گفت‌وگو از کلید API استفاده کنید.</p>
                                <p>
                                    لطفاً <b>&quot;بدون احراز هویت&quot;</b> را از منوی کشویی در گوشه بالا سمت راست انتخاب کنید.
                                </p>
                            </>
                        )}
                        {codeLang === 'جاسازی' && !chatflowApiKeyId && <EmbedChat chatflowid={dialogProps.chatflowid} />}
                        {codeLang !== 'جاسازی' && codeLang !== 'اشتراک‌گذاری ربات گفت‌وگو' && (
                            <>
                                <CopyBlock
                                    theme={atomOneDark}
                                    ...
                                    wrapLines
                                />
                                <CheckboxInput label='نمایش پیکربندی جایگزین' value={checkboxVal} onChange={onCheckBoxChanged} />
                                {checkboxVal && getConfigApi.data && getConfigApi.data.length > 0 && (
                                    <>
                                        <Typography sx={{ mt: 2 }}>
                                            می‌توانید پیکربندی ورودی موجود ربات گفت‌وگو را با ویژگی overrideConfig بازنویسی کنید.
                                        </Typography>
                                        <div
                                            style={{
                                                display: 'flex',
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <span style={{ color: 'rgb(116,66,16)', marginLeft: 10, fontWeight: 500 }}>
                                                    {
                                                        'به دلایل امنیتی، پیکربندی جایگزین به‌طور پیش‌فرض غیرفعال است. می‌توانید این را با رفتن به پیکربندی ربات گفت‌وگو -> تب امنیت تغییر دهید و ویژگی‌ای که می‌خواهید بازنویسی کنید را فعال کنید.'
                                                    }
                                        ...
                                        <Stack direction='column' spacing={2} sx={{ width: '100%', my: 2 }}>
                                            <Card sx={{ borderColor: theme.palette.primary[200] + 75, p: 2 }} variant='outlined'>
                                                <Stack sx={{ mt: 1, mb: 2, ml: 1, alignItems: 'center' }} direction='row' spacing={2}>
                                                    <IconBox />
                                                    <Typography variant='h4'>گره‌ها</Typography>
                                                </Stack>
                                            ...
                                            <Card sx={{ borderColor: theme.palette.primary[200] + 75, p: 2 }} variant='outlined'>
                                                <Stack sx={{ mt: 1, mb: 2, ml: 1, alignItems: 'center' }} direction='row' spacing={2}>
                                                    <IconVariable />
                                                    <Typography variant='h4'>متغیرها</Typography>
                                                </Stack>
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <span style={{ color: '#2d6a4f', marginLeft: 10, fontWeight: 500 }}>
                                                    همچنین می‌توانید چندین مقدار برای یک پارامتر پیکربندی با مشخص کردن شناسه گره تعیین کنید
                                                </span>
                                            </div>
                                ...
                                {getIsChatflowStreamingApi.data?.isStreaming && (
                                    <p>
                                        برای اطلاعات بیشتر در مورد نحوه جریان‌دهی پاسخ به برنامه، به{' '}
                        ...
                        {codeLang === 'اشتراک‌گذاری ربات گفت‌وگو' && !chatflowApiKeyId && (
