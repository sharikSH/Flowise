...
const DocumentStoreDetails = () => {
    ...
    const onDocStoreDelete = async (type, file, removeFromVectorStore) => {
        ...
        if (type === 'STORE') {
            ...
            try {
                ...
                if (deleteResp.data) {
                    enqueueSnackbar({
                        message: 'مخزن، بارگذار و تکه‌های مستند مرتبط حذف شدند',
                        options: {
            ...
            try {
                ...
                if (deleteResp.data) {
                    enqueueSnackbar({
                        message: 'بارگذار و تکه‌های مستند مرتبط حذف شدند',
                        options: {
                            key: new Date().getTime() + Math.random(),
                            variant: 'success',
                            action: (key) => (
                ...
                setBackdropLoading(false)
                enqueueSnackbar({
                    message: `خطا در حذف بارگذار مستند: ${
                        typeof error.response.data === 'object' ? error.response.data.message : error.response.data
                    }`,
                    options: {
                        key: new Date().getTime() + Math.random(),
    ...
    const onLoaderDelete = (file, vectorStoreConfig, recordManagerConfig) => {
        const props = {
            title: `حذف`,
            description: `حذف بارگذار ${file.loaderName}؟ این کار تمام تکه‌های مستند مرتبط را حذف خواهد کرد.`,
            vectorStoreConfig,
            recordManagerConfig,
            type: 'LOADER',
            file
    ...
    const onStoreDelete = (vectorStoreConfig, recordManagerConfig) => {
        const props = {
            title: `حذف`,
            description: `حذف مخزن ${getSpecificDocumentStore.data?.name}؟ این کار تمام بارگذارها و تکه‌های مستند مرتبط را حذف خواهد کرد.`,
            vectorStoreConfig,
            recordManagerConfig,
            type: 'STORE'
        }
    ...
    const onStoreRefresh = async (storeId) => {
        const confirmPayload = {
            title: `تازه‌سازی همه بارگذارها و درج همه تکه‌ها؟`,
            description: `این کار همه بارگذارها را دوباره پردازش کرده و همه تکه‌ها را درج می‌کند. این عملیات ممکن است مدتی طول بکشد.`,
            confirmButtonName: 'تازه‌سازی',
        ...
        if (isConfirmed) {
            ...
            try {
                ...
                if (resp.data) {
                    enqueueSnackbar({
                        message: 'مخزن مستند با موفقیت تازه‌سازی شد!',
                        options: {
                            key: new Date().getTime() + Math.random(),
                            variant: 'success',
                            action: (key) => (
                ...
                setBackdropLoading(false)
                enqueueSnackbar({
                    message: `خطا در تازه‌سازی مخزن مستند: ${
                        typeof error.response.data === 'object' ? error.response.data.message : error.response.data
                    }`,
                    options: {
                        key: new Date().getTime() + Math.random(),
    ...
    const onEditClicked = () => {
        ...
        const dialogProp = {
            title: 'ویرایش مخزن مستند',
            type: 'EDIT',
            cancelButtonName: 'لغو',
            confirmButtonName: 'به‌روزرسانی',
            data: data
        }
                    ...
                    <Stack flexDirection='column' sx={{ gap: 3 }}>
                            ...
                            >
                                ...
                                <Available permission={'documentStores:upsert-config'}>
                                    <MenuItem
                                        ...
                                        disableRipple
                                        title='دوباره پردازش همه بارگذارها و درج همه تکه‌ها'
                                    >
                                        <RefreshIcon />
                                        تازه‌سازی
                                    </MenuItem>
                        ...
                        {!isLoading && documentStore && !documentStore?.loaders?.length ? (
                            <Stack sx={{ alignItems: 'center', justifyContent: 'center' }} flexDirection='column'>
                                <Box sx={{ p: 2, height: 'auto' }}>
                                    <img
                                        ...
                                        alt='تصویر خالی مخزن مستند'
                                    />
                                </Box>
                                <div>هنوز هیچ مستندی اضافه نشده است</div>
                                <StyledButton
                                    variant='contained'
                            ...
                            <TableContainer
                                ...
                                component={Paper}
                            >
                                <Table sx={{ minWidth: 650 }} aria-label='جدول ساده'>
                                    <TableHead
                                        sx={{
                                    ...
                                    >
                                        <TableRow>
                                            ...
                                            <StyledTableCell>بارگذار</StyledTableCell>
                                            <StyledTableCell>تقسیم‌کننده</StyledTableCell>
                                            <StyledTableCell>منبع(ها)</StyledTableCell>
                                            <StyledTableCell>تکه‌ها</StyledTableCell>
                                            <StyledTableCell>کاراکترها</StyledTableCell>
                                            <Available permission={'documentStores:preview-process,documentStores:delete-loader'}>
                                                <StyledTableCell>اقدامات</StyledTableCell>
                                            </Available>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                        ...
                        {getSpecificDocumentStore.data?.status === 'STALE' && (
                            <div style={{ width: '100%', textAlign: 'center', marginTop: '20px' }}>
                                ...
                                >
                                    برخی فایل‌ها در حال پردازش هستند. لطفاً برای دریافت آخرین وضعیت، تازه‌سازی کنید.
                                </Typography>
                            </div>
                        )}
                    </Stack>
    ...
    return (
        <>
            <TableRow hover key={props.index} sx={{ '&:last-child td, &:last-child th': { border: 0 }, cursor: 'pointer' }}>
                ...
                <StyledTableCell onClick={props.onViewChunksClick}>{props.loader.splitterName ?? 'هیچ‌کدام'}</StyledTableCell>
