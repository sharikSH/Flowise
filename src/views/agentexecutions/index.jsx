...
const AgentExecutions = () => {
    ...
    }, [deleteExecutionsApi.data, currentPage, pageLimit, getAllExecutions])
    ...
    }, [getExecutionByIdApi.data, selectedMetadata.agentflow])
        ...
        <MainCard>
            {error ? (
                ...
                <Stack flexDirection='column' sx={{ gap: 3 }}>
                    <ViewHeader title='نشست های فعال' description='نظارت و مدیریت اجرای مولتی ایجنت' />
                    {/* بخش فیلترها */}
                    <Box sx={{ mb: 2, width: '100%' }}>
                        <Grid container spacing={2} alignItems='center'>
                            <Grid item xs={12} md={2}>
                                <FormControl fullWidth size='small'>
                                    <InputLabel id='state-select-label'>وضعیت</InputLabel>
                                    <Select
                                    ...
                                    >
                                        ...
                                        <MenuItem value='FINISHED'>پایان‌یافته</MenuItem>
                                        <MenuItem value='ERROR'>خطا</MenuItem>
                                        <MenuItem value='TERMINATED'>خاتمه‌یافته</MenuItem>
                                        <MenuItem value='TIMEOUT'>تایم‌اوت</MenuItem>
                                        <MenuItem value='STOPPED'>متوقف‌شده</MenuItem>
                                    </Select>
                                </FormControl>
                            </Grid>
                            <Grid item xs={12} md={2}>
                            ...
                            <Grid item xs={12} md={4}>
                                <Stack direction='row' spacing={1}>
                                    ...
                                    <Available permissions={['executions:delete']}>
                                        <Tooltip title='حذف نشست های انتخاب‌شده'>
                                            <span>
                                                <IconButton
                    ...
                    {executions?.length > 0 && (
                        <>
                            ...
                            {/* کنترل‌های صفحه‌بندی و اندازه صفحه */}
                            {!isLoading && total > 0 && (
                    ...
                    >
                        <DialogTitle id='alert-dialog-title'>تأیید حذف</DialogTitle>
                        <DialogContent>
                            <DialogContentText id='alert-dialog-description'>
                                آیا مطمئن هستید که می‌خواهید {selectedExecutionIds.length} اجرا را حذف کنید
                                {selectedExecutionIds.length !== 1 ? '' : ''}؟ این اقدام قابل بازگشت نیست.
                            </DialogContentText>
                        </DialogContent>
                        <DialogActions>
                    ...
                    {!isLoading && (!executions || executions.length === 0) && (
                        <Stack sx={{ alignItems: 'center', justifyContent: 'center' }} flexDirection='column'>
                            <Box sx={{ p: 2, height: 'auto' }}>
                                <img style={{ objectFit: 'cover', height: '20vh', width: 'auto' }} src={execution_empty} alt='اجرای_خالی' />
                            </Box>
                            <div>هنوز اجرایی وجود ندارد</div>
                        </Stack>
                    )}
                </Stack>
            )}
