...
const Canvas = () => {
    ...
    const canvasTitle = URLpath.includes('agentcanvas') ? 'ایجنت' : 'جریان گفتگو'
    ...
    const handleDeleteFlow = async () => {
        const confirmPayload = {
            title: `حذف`,
            description: `آیا مایل به حذف ${canvasTitle} ${chatflow.name} هستید؟`,
    ...
    useEffect(() => {
        if (getSpecificChatflowApi.data) {
            ...
            errorFailed(`بازیابی ${canvasTitle} ناموفق بود: ${getSpecificChatflowApi.error.response.data.message}`)
        }
    ...
    useEffect(() => {
        if (createNewChatflowApi.data) {
            ...
            errorFailed(`بازیابی ${canvasTitle} ناموفق بود: ${createNewChatflowApi.error.response.data.message}`)
        }
    ...
    useEffect(() => {
        if (updateChatflowApi.data) {
            ...
            errorFailed(`بازیابی ${canvasTitle} ناموفق بود: ${updateChatflowApi.error.response.data.message}`)
        }
    ...
    useEffect(() => {
        const checkIfHasChanged = async () => {
            if (getHasChatflowChangedApi.data?.hasChanged === true) {
                const confirmPayload = {
                    title: `تأیید تغییر`,
                    description: `${canvasTitle} ${chatflow.name} از زمانی که باز کرده‌اید تغییر کرده است، تغییرات فعلی را جایگزین کنیم؟`,
                    confirmButtonName: 'تأیید',
                    cancelButtonName: 'لغو'
                }
