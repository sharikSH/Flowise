...
const NavGroup = ({ item, drawerOpen }) => {
    ...
    const listItems = (menu, level = 1) => {
        ...
        switch (menu.type) {
            case 'collapse':
                return <NavCollapse key={menu.id} menu={menu} level={level} drawerOpen={drawerOpen} />
            case 'item':
                return <NavItem key={menu.id} item={menu} level={level} navType='MENU' drawerOpen={drawerOpen} />
            default:
                return (
    ...
    const renderNonPrimaryGroups = () => {
        ...
        return nonprimaryGroups.filter((group) => group.children.length > 0)
    }
