...
import { IconFileExport, IconFileUpload, IconInfoCircle, IconLogout, IconUserEdit, IconX, IconSettings } from '@tabler/icons-react'
import './index.css'
    ...
    const component = show ? (
        ...
        >
            <DialogTitle sx={{ fontSize: '1rem' }} id='export-dialog-title'>
                {!isExporting ? 'انتخاب داده برای صدور' : 'در حال صدور...'}
            </DialogTitle>
            <DialogContent>
                {!isExporting && (
                    <Stack
                    ...
                    <Box sx={{ height: 'auto', display: 'flex', justifyContent: 'center', mb: 3 }}>
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            <img
                                ...
                                src={ExportingGIF}
                                alt='تصویر در حال صدور'
                            />
                            <span>صدور داده ممکن است مدتی طول بکشد</span>
                        </div>
                    </Box>
                )}
            </DialogContent>
                ...
                <Box sx={{ height: 'auto', display: 'flex', justifyContent: 'center', mb: 3 }}>
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <img
                            ...
                            src={ExportingGIF}
                            alt='تصویر در حال واردسازی'
                        />
                        <span>واردسازی داده ممکن است مدتی طول بکشد</span>
                    </div>
                </Box>
            </DialogContent>
        </Dialog>
...
const ProfileSection = ({ handleLogout }) => {
    ...
    const importAllSuccess = () => {
        ...
        enqueueSnackbar({
            message: `واردسازی همه با موفقیت انجام شد`,
            options: {
                key: new Date().getTime() + Math.random(),
                variant: 'success',
                action: (key) => (
    ...
    const onExport = (data) => {
        const body = {}
        if (data.includes('مولتی ایجنت')) body.agentflow = true
        if (data.includes('مولتی ایجنت نسخه ۲')) body.agentflowv2 = true
        if (data.includes('دستیارهای سفارشی')) body.assistantCustom = true
        if (data.includes('دستیارهای OpenAI')) body.assistantOpenAI = true
        if (data.includes('دستیارهای Azure')) body.assistantAzure = true
        if (data.includes('ایجنت نویسی')) body.chatflow = true
        if (data.includes('پیام‌های گفتگو')) body.chat_message = true
        if (data.includes('بازخوردهای گفتگو')) body.chat_feedback = true
        if (data.includes('قالب‌های سفارشی')) body.custom_template = true
        if (data.includes('مخازن اسناد')) body.document_store = true
        if (data.includes('نشست ها')) body.execution = true
        if (data.includes('ابزارها')) body.tool = true
        if (data.includes('متغیرها')) body.variable = true
    ...
    useEffect(() => {
        if (exportAllApi.data) {
            ...
            try {
                ...
                errorFailed(`خطا در صدور همه: ${getErrorMessage(error)}`)
            }
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [exportAllApi.data])
    ...
    useEffect(() => {
        if (exportAllApi.error) {
            ...
            errorFailed(`خطا در صدور: ${errMsg}`)
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [exportAllApi.error])
    ...
    return (
        <>
            ...
            <Popper
                placement={theme.direction === 'rtl' ? 'bottom-start' : 'bottom-end'} // Dynamic placement for RTL
                open={open}
